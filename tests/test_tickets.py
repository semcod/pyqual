...
def test_sync_todo_tickets_uses_planfile_markdown_backend(
    ...
    monkeypatch.setattr(
        tickets_module, "_load_sync_integration", lambda: fake_sync_integration
    )
...
def test_sync_github_tickets_uses_planfile_github_backend(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    calls: list[tuple[str, str, bool, str, bool]] = []
    ...
    tickets_module.sync_github_tickets(
        workdir=tmp_path, dry_run=False, direction="both"
    )

    assert calls == [("github", str(tmp_path), False, "both", True)]


def test_sync_all_tickets_calls_both_backends(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    calls: list[tuple[str, str, bool, str, bool]] = []
...
def test_tickets_todo_cli_invokes_sync_helper(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    def fake_sync_todo_tickets(*, workdir: Path, dry_run: bool, direction: str) -> None:
        captured.update(
            {"workdir": workdir, "dry_run": dry_run, "direction": direction}
        )
...
def test_run_on_fail_create_ticket_syncs_todo_tickets(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
"""
    ...
    def fake_sync_planfile_tickets(
        source: str,
        workdir: Path,
        dry_run: bool,
        direction: str,
    ...
    monkeypatch.setattr(
        tickets_mod, "sync_planfile_tickets", fake_sync_planfile_tickets
    )
...
def test_run_report_includes_todo_and_fix_summary(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    config_path = tmp_path / "pyqual.yaml"
    config_path.write_text(
