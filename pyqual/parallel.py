...
class ParallelExecutor:
    ...
    def _run_tool_task(self, tool: FixTool, task_id: int, issue: str) -> TaskResult:
        ...
        log.info(
            "parallel task=%d tool=%s issue=%r",
            task_id,
            tool.name,
            issue[:LOG_DETAIL_MAX_LEN],
        )
        ...
        log.info(
            "parallel task=%d tool=%s success=%s duration=%.1fs",
            task_id,
            tool.name,
            result.success,
            result.duration,
    ...
    def run(self, issues: list[str], group_similar: bool = True) -> ParallelRunResult:
        ...
        log.info(
            "parallel_done tasks=%d completed=%d failed=%d duration=%.1fs",
            total_tasks,
            completed,
            failed,
            duration,
