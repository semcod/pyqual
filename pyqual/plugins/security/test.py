    ...
    def test_bandit_not_found(self, tmp_path: Path):
        ...
        if not result["success"]:
            assert (
                ...
                or "bandit" in result.get("error", "").lower()
            )
...
class TestPipAudit:
    ...
    def test_pip_audit_not_found(self):
        ...
        if not result["success"]:
            assert (
                "not found" in result.get("error", "").lower()
                or "pip-audit" in result.get("error", "").lower()
            )
...
class TestDetectSecrets:
    ...
    def test_detect_secrets_not_found(self):
        ...
        if not result["success"]:
            assert (
                "not found" in result.get("error", "").lower()
                or "detect-secrets" in result.get("error", "").lower()
            )
...
class TestSecuritySummary:
    ...
    def test_security_summary_with_issues(self, tmp_path: Path):
        ...
        bandit_data = {"results": [{"issue_severity": "HIGH"}], "metrics": {}}
        (pyqual_dir / "bandit.json").write_text(json.dumps(bandit_data))
