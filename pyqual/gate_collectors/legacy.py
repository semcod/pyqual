...
def _from_bandit(workdir: Path) -> dict[str, float]:
    ...
    if p.exists():
        try:
            ...
            result["bandit_high"] = float(
                sum(1 for r in results if r.get("issue_severity") == "HIGH")
            )
            result["bandit_medium"] = float(
                sum(1 for r in results if r.get("issue_severity") == "MEDIUM")
            )
            result["bandit_low"] = float(
                sum(1 for r in results if r.get("issue_severity") == "LOW")
            )
        except (json.JSONDecodeError, TypeError, KeyError):
            pass
    return result
...
def _from_secrets(workdir: Path) -> dict[str, float]:
    ...
    try:
        ...
        if isinstance(data, list):
            ...
            count = sum(
                len(v) for v in data.get("results", {}).values() if isinstance(v, list)
            )
...
def _from_vulnerabilities(workdir: Path) -> dict[str, float]:
    ...
    try:
        ...
        if isinstance(data, list):
            result["vuln_critical"] = float(
                sum(1 for v in data if v.get("severity", "").lower() == "critical")
            )
            result["vuln_high"] = float(
                sum(1 for v in data if v.get("severity", "").lower() == "high")
            )
            result["vuln_medium"] = float(
                sum(1 for v in data if v.get("severity", "").lower() == "medium")
            )
    except (json.JSONDecodeError, TypeError):
        pass
    return result
