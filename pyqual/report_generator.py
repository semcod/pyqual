...
def _build_gate(
    metric: str, value: float, threshold: float, operator: str
) -> dict[str, Any]:
...
def _build_gates_from_metrics(metrics: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for metric_key, gate_name, threshold, operator in gate_configs:
        if metric_key in metrics:
            gates.append(
                _build_gate(gate_name, metrics[metric_key], threshold, operator)
            )
    return gates
...
def _build_run_from_rows(
    rows: list, latest_timestamp: str, db_path: "Path"
) -> "PipelineRun":
    ...
    all_gates_passed = (
        all(gate.get("passed", False) for gate in gates) if gates else True
    )
    stages.reverse()
    return PipelineRun(
...
def generate_ascii_diagram(run: PipelineRun) -> str:
    ...
    for stage in run.stages:
        ...
        lines.append(
            f"│  {status_icon} {stage.name:<25} {stage.duration:>6.1f}s {status_color}        │"
        )
    ...
    lines.append(
        f"│  ⏱️  Total time: {run.total_time:.1f}s                                          │"
    )
    lines.append("└─────────────────────────────────────────────────────────────────┘")
    ...
    for stage in run.stages:
        ...
        if stage.details:
            ...
            if "vallm_pass_pct" in details:
                lines.append(
                    f"- **vallm pass %:** {details.get('vallm_pass_pct', 0):.1f}%"
                )
...
def generate_report(workdir: Path | None = None) -> str:
    ...
    if run.all_gates_passed:
        lines.append(
            f"✅ **All quality gates passed!** Pipeline completed successfully in {run.total_time:.1f}s."
        )
    else:
        lines.append(
