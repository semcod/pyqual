...
def git_status_cmd(
    ...
    if result["staged_files"]:
        console.print(f"\n[green]Staged ({len(result['staged_files'])}):[/green]")
        for f in result["staged_files"][:10]:
            console.print(f"  + {f}")
        if len(result["staged_files"]) > 10:
            console.print(f"  ... and {len(result['staged_files']) - 10} more")
    
    if result["unstaged_files"]:
        console.print(f"\n[yellow]Unstaged ({len(result['unstaged_files'])}):[/yellow]")
        for f in result["unstaged_files"][:10]:
            console.print(f"  M {f}")
        if len(result["unstaged_files"]) > 10:
            console.print(f"  ... and {len(result['unstaged_files']) - 10} more")
    
    if result["untracked_files"]:
        console.print(f"\n[red]Untracked ({len(result['untracked_files'])}):[/red]")
        for f in result["untracked_files"][:10]:
            console.print(f"  ? {f}")
        if len(result["untracked_files"]) > 10:
            console.print(f"  ... and {len(result['untracked_files']) - 10} more")
