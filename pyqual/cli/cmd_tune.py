...
def tune_thresholds(
    aggressive: bool = typer.Option(
        False, "--aggressive", "-a", help="More ambitious thresholds"
    ),
    conservative: bool = typer.Option(
        False, "--conservative", "-c", help="Safer thresholds with margin"
    ),
    dry_run: bool = typer.Option(
        False, "--dry-run", "-d", help="Show changes without applying"
    ),
    config_path: Path = typer.Option(
        Path("pyqual.yaml"), "--config", "-f", help="Config file path"
    ),
) -> None:
    """
    ...
    suggested = _calculate_thresholds(
        latest_metrics, aggressive=aggressive, conservative=conservative
    )
    ...
    if dry_run or (not aggressive and not conservative):
        ...
        console.print(
            "[dim]To apply changes, the tune command will update pyqual.yaml[/dim]"
        )
    else:
        if _confirm_apply():
...
def _calculate_thresholds(
    ...
    if "ruff_errors" in metrics:
        suggested["ruff_errors_max"] = max(
            500, int(metrics["ruff_errors"] * upper_margin)
        )
    ...
    if "documentation_score" in metrics:
        suggested["documentation_score_min"] = int(
            metrics["documentation_score"] * lower_margin
        )
    if "readme_completeness" in metrics:
        suggested["readme_completeness_min"] = int(
            metrics["readme_completeness"] * lower_margin
        )
    ...
    if "pyproject_completeness" in metrics:
        suggested["pyproject_completeness_min"] = min(
            90, int(metrics["pyproject_completeness"] * lower_margin)
        )
...
def _display_comparison(
    current: dict[str, Any], suggested: dict[str, Any], actual: dict[str, float]
) -> None:
    ...
    for metric, suggested_val in suggested.items():
        ...
        base_name = (
            metric.replace("_min", "").replace("_max", "").replace("_exists", "")
        )
        actual_val = actual.get(base_name, actual.get(metric, "—"))

        # Determine if threshold would pass
        if isinstance(actual_val, (int, float)) and isinstance(
            suggested_val, (int, float)
        ):
