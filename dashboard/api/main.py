...
def read_summary_json(project_id: str) -> Optional[Dict[str, Any]]:
    ...
    summary_path = Path(
        f"{PROJECTS_BASE_DIR}/{project_id}/{PYQUAL_SUBDIR}/{SUMMARY_JSON_NAME}"
    )
...
async def get_latest_run(project_id: str):
    ...
    if not summary:
        raise HTTPException(
            status_code=HTTP_NOT_FOUND, detail="Project not found or no runs available"
        )
    return summary
...
async def get_metric_history(
    project_id: str, metric: str, days: int = DEFAULT_METRIC_HISTORY_DAYS
):
    ...
    for row in query_pipeline_db(db_path, query, (pattern, threshold)):
        ...
        if kwargs.get("metric") == metric:
            history.append(
                {"timestamp": row["timestamp"], "value": kwargs.get("value", 0)}
            )
...
async def get_stage_performance(
    project_id: str, days: int = DEFAULT_STAGE_PERFORMANCE_DAYS
):
...
async def ingest_results(
    ...
    if credentials.credentials != INGEST_TOKEN:
        raise HTTPException(
            status_code=HTTP_UNAUTHORIZED, detail="Invalid authentication token"
        )
