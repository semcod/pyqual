...
- **Calls**: log.info, time.monotonic, self._log_stage, self._resolve_tool_stage, StageResult, self._log_stage, subprocess.run, StageResult
...
### pyqual._gate_collectors._from_sbom
...
### pyqual._gate_collectors._from_pyroma
...
### pyqual.builtin_collectors.RepoMetricsCollector
> Advanced repository health metrics (bus factor, diversity).
- **Methods**: 1
- **Key Methods**: pyqual.builtin_collectors.RepoMetricsCollector.collect
- **Inherits**: MetricCollector
...
- **Output to**: line.strip, clean.split, len, None.strip, int
...
- `pyqual.cli.bulk_run_cmd` - 52 calls
...
- `pyqual.bulk_run.build_dashboard_table` - 17 calls
...
- `run_analysis.main` - 9 calls
- `pyqual.builtin_collectors.I18nCollector.collect` - 9 calls
